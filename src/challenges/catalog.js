import { customChallenges } from './custom.js'
...
const projectScenarioFamilies = [
  {
    kind: 'explore', title: 'Orient yourself in the codebase',
    prompt: ({ label }) => `List the top-level ${label} files, then read README.md before making any changes.`,
    commands: () => ['ls', 'cat README.md'], explanation: 'A quick inventory and project overview are a safe start in an unfamiliar repository.',
  },
  {
  ...
  {
    kind: 'review-staged', title: 'Review the proposed snapshot',
    prompt: ({ label, file }) => `${file} is staged in ${label}. Review only its staged patch before deciding what comes next.`,
    commands: ({ file }) => [`git diff --staged -- ${file}`], explanation: 'A path-limited staged diff shows exactly what the next snapshot would contain.',
  },
  {
    kind: 'unstage', title: 'Remove the wrong file from staging',
    prompt: ({ label, file }) => `${file} was staged too early in ${label}. Unstage it without losing the working-tree edit.`,
    commands: ({ file }) => [`git restore --staged ${file}`], explanation: 'Restoring the index leaves the edited working copy intact.',
  },
  {
    kind: 'selective-commit', title: 'Commit one concern only',
    prompt: ({ label, file, message }) => `${label} has edits in ${file} and notes.txt. Commit only ${file} with the message “${message}” and leave the notes uncommitted.`,
    commands: ({ file, message }) => [`git add ${file}`, `git commit -m "${message}"`], explanation: 'Selective staging produces a focused commit while preserving unrelated work.',
  },
  {
    kind: 'stash-untracked', title: 'Pause a spike with new files',
    prompt: ({ label, stashMessage }) => `${label} has tracked edits and a new spike-notes.md file. Stash both with the message “${stashMessage}”.`,
    commands: ({ stashMessage }) => [`git stash push -u -m "${stashMessage}"`], explanation: 'The -u flag includes untracked files in the saved work.',
  },
  {
    kind: 'stash-restore', title: 'Resume interrupted work',
    prompt: ({ label, stashMessage }) => `The latest ${label} stash is labeled “${stashMessage}”. Restore it and remove that stash entry.`,
    commands: () => ['git stash pop'], explanation: 'Stash pop reapplies the latest saved work and drops it when application succeeds.',
  },
  {
    kind: 'rename-branch', title: 'Correct a branch name',
    prompt: ({ branch }) => `The current branch is still named scratch. Rename it to ${branch}.`,
    commands: ({ branch }) => [`git branch -m ${branch}`], explanation: 'Renaming the current branch keeps its commits and working tree unchanged.',
  },
  {
    kind: 'delete-branch', title: 'Remove a merged branch',
    prompt: ({ cleanupBranch, label }) => `${cleanupBranch} is fully merged into ${label}’s main branch. Delete the local branch safely.`,
    commands: ({ cleanupBranch }) => [`git branch -d ${cleanupBranch}`], explanation: 'Lowercase -d refuses to delete a branch whose commits are not merged.',
  },
  {
    kind: 'compare-branch', title: 'Compare a feature with main',
    prompt: ({ branch, file }) => `Review how ${file} differs between main and ${branch}, including the feature branch’s commits.`,
    commands: ({ branch, file }) => [`git diff main...${branch} -- ${file}`], explanation: 'The three-dot diff compares the feature tip with its merge base on main.',
  },
  {
    kind: 'merge-branch', title: 'Integrate completed feature work',
    prompt: ({ branch, label }) => `${branch} is reviewed and main is checked out. Merge the branch into ${label}.`,
    commands: ({ branch }) => [`git merge ${branch}`], explanation: 'Merging incorporates the reviewed branch history into the current branch.',
  },
  {
    kind: 'sync-rebase', title: 'Rebase onto updated remote main',
    prompt: ({ branch }) => `Fetch origin, then rebase the current ${branch} branch onto origin/main.`,
    commands: () => ['git fetch origin', 'git rebase origin/main'], explanation: 'Fetch refreshes the remote-tracking branch before the feature commits are replayed.',
  },
  {
    kind: 'pull-update', title: 'Bring main up to date',
    prompt: ({ label }) => `${label}’s local main is behind origin/main. Pull and integrate the remote update.`,
    commands: () => ['git pull origin main'], explanation: 'Pull fetches the selected remote branch and integrates it into the current branch.',
  },
  {
    kind: 'publish-branch', title: 'Publish a finished branch',
    prompt: ({ branch }) => `Publish ${branch} to origin and configure its upstream in the same command.`,
    commands: ({ branch }) => [`git push -u origin ${branch}`], explanation: 'Setting an upstream makes later pulls and pushes shorter and less ambiguous.',
  },
  {
    kind: 'clean-ignored', title: 'Remove generated output safely',
    prompt: ({ label }) => `${label} contains ignored dist and coverage output. Preview the ignored cleanup, then remove only those ignored files.`,
    commands: () => ['git clean -ndX', 'git clean -fdX'], explanation: 'A dry run followed by -X removes ignored build output without touching untracked source files.',
  },
  {
    kind: 'show-file', title: 'Read the committed file',
    prompt: ({ file }) => `Display ${file} exactly as it exists in HEAD without changing the working tree.`,
    commands: ({ file }) => [`git show HEAD:${file}`], explanation: 'A revision:path expression reads a committed file independently of local edits.',
  },
  {
    kind: 'show-commit', title: 'Inspect the latest snapshot',
    prompt: ({ label }) => `Show ${label}’s latest commit with a compact heading and file statistics.`,
    commands: () => ['git show --stat --oneline HEAD'], explanation: 'Commit statistics reveal the scope of the newest snapshot without printing every patch line.',
  },
  {
    kind: 'branch-list', title: 'Survey active lines of work',
    prompt: ({ label }) => `List ${label}’s local branches and identify which one is currently checked out.`,
    commands: () => ['git branch'], explanation: 'The local branch list marks the current branch with an asterisk.',
  },
  {
    kind: 'remote-list', title: 'Inspect repository connections',
    prompt: ({ label }) => `Show every configured ${label} remote and its fetch and push URLs.`,
    commands: () => ['git remote -v'], explanation: 'Inspecting remote URLs confirms where fetches and pushes will go.',
  },
  {
    kind: 'review-commit', title: 'Review and record a staged fix',
    prompt: ({ label, file, message }) => `${file} is staged in ${label}. Review that staged path, then commit it with the message “${message}”.`,
    commands: ({ file, message }) => [`git diff --staged -- ${file}`, `git commit -m "${message}"`], explanation: 'Reviewing the exact staged path before committing catches accidental snapshot contents.',
  },
  {
    kind: 'stash-list', title: 'Find saved work',
    prompt: ({ label, stashMessage }) => `${label} has shelved work named “${stashMessage}”. List the stash entries so you can locate it.`,
    commands: () => ['git stash list'], explanation: 'The stash list shows saved work with its branch context and message.',
  },
  {
    kind: 'review-stage-commit', title: 'Review, stage, and commit a fix',
    prompt: ({ label, file, message }) => `Review the unstaged ${file} fix in ${label}, stage it, then commit with the message “${message}”.`,
    commands: ({ file, message }) => [`git diff -- ${file}`, `git add ${file}`, `git commit -m "${message}"`],
    explanation: 'Reviewing before staging and committing creates a deliberate, auditable snapshot.',
  },
  {
    kind: 'discard-stage-commit', title: 'Separate a fix from scratch work',
    prompt: ({ label, file, message }) => `${label} has a valid ${file} fix and an accidental notes.txt edit. Restore the notes, stage the fix, and commit it as “${message}”.`,
    commands: ({ file, message }) => ['git restore notes.txt', `git add ${file}`, `git commit -m "${message}"`],
    explanation: 'Discarding the accidental edit before staging keeps the final commit focused.',
  },
  {
    kind: 'branch-stage-commit', title: 'Move new work onto a feature branch',
    prompt: ({ label, file, branch, message }) => `${file} was edited on ${label}’s main branch but is not committed. Create and switch to ${branch}, stage the file, and commit it as “${message}”.`,
    commands: ({ file, branch, message }) => [`git switch -c ${branch}`, `git add ${file}`, `git commit -m "${message}"`],
    explanation: 'Creating the branch before committing moves the new snapshot off main without losing the working edit.',
  },
  {
    kind: 'fetch-review-rebase', title: 'Review upstream before rebasing',
    prompt: ({ branch }) => `Fetch origin, review the commits that origin/main has and ${branch} does not, then rebase ${branch} onto origin/main.`,
    commands: () => ['git fetch origin', 'git log --oneline HEAD..origin/main', 'git rebase origin/main'],
    explanation: 'Inspecting fetched upstream commits makes the subsequent rebase an informed choice.',
  },
  {
    kind: 'merge-cleanup', title: 'Merge and remove a completed branch',
    prompt: ({ label, branch }) => `${branch} is reviewed and ${label} main is checked out. Merge it, then delete the now-merged local branch.`,
    commands: ({ branch }) => [`git merge ${branch}`, `git branch -d ${branch}`],
    explanation: 'Deleting a merged branch keeps local navigation tidy without losing its history.',
  },
  {
    kind: 'tag-publish', title: 'Create and publish a release tag',
    prompt: ({ label, tag }) => `Create ${label}’s annotated ${tag} tag with the message “Release ${tag}”, then push only that tag to origin.`,
    commands: ({ tag }) => [`git tag -a ${tag} -m "Release ${tag}"`, `git push origin ${tag}`],
    explanation: 'Creating and explicitly pushing a tag turns a local release marker into a published one.',
  },
  {
    kind: 'resume-stash', title: 'Inspect and resume saved work',
    prompt: ({ label, stashMessage }) => `${label} has a stash named “${stashMessage}”. Confirm it is present, then pop it back into the working tree.`,
    commands: () => ['git stash list', 'git stash pop'],
    explanation: 'Listing before popping verifies which saved work is about to be restored.',
  },
  {
    kind: 'review-publish', title: 'Review and publish a feature branch',
    prompt: ({ branch }) => `Review the commits on ${branch} that are not on origin/main, then push the branch and set its upstream.`,
    commands: ({ branch }) => ['git log --oneline origin/main..HEAD', `git push -u origin ${branch}`],
    explanation: 'A final commit review before publishing catches scope problems before collaborators see the branch.',
  },
]

export const projectChallenges = projectProfiles.flatMap((profile, profileIndex) => {
  const context = {
    ...profile,
    branch: `feature/${profile.slug.replace(/-(cli|web|api|docs)$/, '')}-improvements`,
    cleanupBranch: `chore/${profile.slug}-cleanup`,
    message: `Update ${profile.area}`,
    stashMessage: `wip ${profile.area}`,
    tag: `v1.${profileIndex + 1}.0`,
  }
  return Array.from({ length: 8 }, (_, scenarioIndex) => projectScenarioFamilies[(profileIndex * 8 + scenarioIndex) % projectScenarioFamilies.length]).map((family) => ({
...
const releaseProjectChallenges = [
  {
    profile: projectProfiles[0], kind: 'release-tag', title: 'Create an annotated release tag',
    prompt: 'Create Telescope CLI’s annotated v1.1.0 tag with the message “Release v1.1.0”.',
    commands: ['git tag -a v1.1.0 -m "Release v1.1.0"'],
    explanation: 'An annotated tag records a release name, creator, date, and message.',
  },
  {
    profile: projectProfiles[1], kind: 'release-branch', title: 'Open a release preparation branch',
    prompt: 'Create and switch to TrailMap Web’s release/1.1 branch.',
    commands: ['git switch -c release/1.1'],
    explanation: 'A release branch isolates final stabilization from ongoing feature work.',
  },
  {
    profile: projectProfiles[2], kind: 'release-history', title: 'Review release history',
    prompt: 'Show Lantern API’s three most recent commits in compact one-line form before drafting release notes.',
    commands: ['git log --oneline -3'],
    explanation: 'A bounded history view provides a concise starting point for release notes.',
  },
  {
    profile: projectProfiles[3], kind: 'publish-tag', title: 'Publish a release tag',
    prompt: 'Pocket Notes already has a local v1.1.0 tag. Push only that tag to origin.',
    commands: ['git push origin v1.1.0'],
    explanation: 'Pushing the explicit tag publishes the release marker without pushing unrelated branches.',
  },
].map((release) => ({
  id: `project-${release.profile.slug}-${release.kind}`,
  title: `${release.title} / ${release.profile.label}`,
  prompt: release.prompt,
  commands: release.commands,
  explanation: release.explanation,
  tip: `This release exercise runs only inside the disposable ${release.profile.label} repository.`,
  scenario: { kind: release.kind, profile: release.profile, tag: 'v1.1.0', branch: 'release/1.1' },
}))

projectChallenges.push(...releaseProjectChallenges, ...customChallenges)
...
export function sampleChallenge(mode, previousId) {
  const previous = challengeById(previousId)
  const previousSolution = previous?.commands.join('\n')
  let options = challengeSets[mode].filter((item) => item.id !== previousId && item.commands.join('\n') !== previousSolution)
  if (!options.length) options = challengeSets[mode].filter((item) => item.id !== previousId)
